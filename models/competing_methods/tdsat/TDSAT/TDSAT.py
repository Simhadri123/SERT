...
class AdaptiveSkipConnection(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # Match spatial sizes if there are minor mismatches from rounding/upsampling
        if x.shape[2:] != y.shape[2:]:  # compare (T, H, W)
            y = F.interpolate(y, size=x.shape[2:], mode='trilinear', align_corners=True)

        # Make memory contiguous before concatenation to avoid CUDA misaligned address
        x = x.contiguous()
        y = y.contiguous()
